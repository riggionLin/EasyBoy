...
@JvmOverloads constructor(
    ...
    var fullReplace: Boolean = true//仅显示1个View。
    ...
    private fun loadingViewGone() {
        if (loadingView != null) {
            if (loadingViewDelayHide) {
                val dataAfter = loadingView?.tag as? Long ?: 0L
                if (dataAfter != 0L) {
                    val dif = System.currentTimeMillis() - dataAfter
                    if (dif < 1000L) {
                        val targetT = 1000L - dif
                        h.postDelayed({
        ...
        when (viewState) {
            VIEW_STATE_LOADING -> {
                requireNotNull(loadingView).apply {
                    contentView?.visibility = if (fullReplace) View.GONE else View.VISIBLE
                    ...
                    if (animateLayoutChanges) {
                        ...
                        loadingView?.tag = System.currentTimeMillis()
                        alpha = 1F
