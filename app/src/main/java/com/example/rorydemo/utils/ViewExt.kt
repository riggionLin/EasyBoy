...
//创建一个LiveData
fun <T> View.toLiveData(change: (t: T) -> Unit): MutableLiveData<T> {
    val l = MutableLiveData<T>()
    l.observe(context as LifecycleOwner, Observer {
        change.invoke(it)
    })
    return l
}

fun <T> MutableLiveData<T>.withDefault(value: T):MutableLiveData<T> {
    postValue(value)
    return this
}
